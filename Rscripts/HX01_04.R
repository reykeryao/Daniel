...
library(Biostrings)
setwd("/stor/work/Lambowitz/yaojun/Work/JA25159_25211_Daniel/Daniel/")
### overall NT frq
pdf("Figs/Nt_frq_total.pdf")
frq<-read.delim("Results/nt_frq.txt",row.names = 1)
...
### polyN distribution
polyN<-read.table("Results/polyN.txt",row.names = 1,header = TRUE)
pdf("Figs/polyN.pdf")
par(mfrow=c(2,2))
##polyA
pick=c(2:10)
...
polyN<-read.table("Results/start_polyN.txt",row.names = 1,header = TRUE)
...
polyN<-read.table("Results/end_polyN.txt",row.names = 1,header = TRUE)
...
polyN<-read.table("Results/polyN_patterns.txt",row.names = 1,header = TRUE)
