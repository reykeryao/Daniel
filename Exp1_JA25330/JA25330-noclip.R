...
for (sample in paste0("DK4_",2:7)){
  ...
  tmp[is.na(tmp)]<-0
  tmp1[is.na(tmp1)]<-0
  colnames(tmp)<-colnames(tmp1)<-c("NT",colnames(dat)[3:52])
  if (sample=="DK4_2"){
...
names(res)
saveRDS(res,"processed_mafft_corrected_noclip.outout")
saveRDS(res1,"processed_mafft_removed_noclip.outout")
'''

res<-readRDS("processed_mafft_corrected_noclip.outout")
...
plot(y=Er[,1],x=1:50,ylim=c(0,6),bty="n",xlim=c(1,50),pch=19,cex=0.5,axes=FALSE,xlab="cDNA positions",ylab="Error rate (%)")
points(y=Er[,2],x=1:50,col="red",pch=19,cex=0.5)
axis(1,at=seq(1,50,1),labels=FALSE)
axis(1,at=1:50,labels=Er$`1`,cex.axis=0.5)
abline(v=26)
